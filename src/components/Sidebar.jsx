...
const navItems = [
  ...
  { name: 'HR Managers', path: '/admin/hr-managers', icon: <ShieldAlert /> },
  { name: 'Employees', path: '/admin/employees', icon: <UserSquare2 /> },
  { name: 'Departments', path: '/admin/departments', icon: <Building2 /> },
  { name: 'Attendance', path: '/admin/attendance', icon: <CalendarCheck2 /> },
  { name: 'Leave Management', path: '/admin/leave-management', icon: <CalendarClock /> },
  { name: 'Payroll', path: '/admin/payroll', icon: <Banknote /> },
  { name: 'Recruitment', path: '/admin/recruitment', icon: <UserSearch /> },
  { name: 'Reports', path: '/admin/reports', icon: <BarChart3 /> },
  { name: 'Conduct Reports', path: '/admin/conduct-reports', icon: <AlertOctagon /> },
  { name: 'Announcements', path: '/admin/announcements', icon: <Megaphone /> },
  { name: 'Settings', path: '/admin/settings', icon: <Settings />, mt: true },
];
      ...
      <div className="flex items-center justify-between mb-10 lg:mb-6 xl:mb-12 w-full">
        <div className="flex items-center gap-4 lg:gap-0 xl:gap-4 min-w-0">
          ...
          <div className="block lg:hidden xl:block min-w-0">
            ...
            <div className="text-[10px] uppercase tracking-widest text-white/50 font-black mt-1 font-label">Admin Panel</div>
          </div>
        </div>
      ...
      <div className="mt-auto pt-6 border-t border-white/10 w-full lg:pt-4 xl:pt-8">
        ...
        <div className="flex items-center gap-4 p-2 rounded-xl bg-white/5 hover:bg-white/10 transition-colors cursor-pointer group
          ...
          <div className="overflow-hidden block lg:hidden xl:block">
            <p className="text-sm font-bold text-white truncate font-headline tracking-tight capitalize">{user?.fullName || user?.name || 'Super Admin'}</p>
            <p className="text-[10px] text-white/50 font-bold truncate uppercase tracking-widest font-label mt-0.5">Super Admin</p>
          </div>
        </div>
      </div>
    </aside>
